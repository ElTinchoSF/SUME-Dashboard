...
from .client import SUMEClient
from .config import ScraperConfig
from .normalizer import get_normalizer, normalize
from .parser import ExpedienteDict, MovimientoDict, parse_listing_page, parse_movimientos_table
...
class SUMEScraper:
    ...
    def _fetch_listing_page(self, page_num: int) -> str | None:
        ...
        if page_num == 1:
            ...
            result = self.client.request(
                "POST", f"{self.config.base_url}buscar/", data=search_params
            )
        else:
    ...
    def _upsert_expediente(
        self, expediente: ExpedienteDict, movimientos: list[MovimientoDict]
    ) -> None:
        """
        ...
        with transaction() as conn:
            ...
            if existing:
                ...
                logger.debug(
                    f"Actualizado expediente {expediente.numero} con {len(movimientos)} movimientos"
                )
                ...
                logger.debug(
                    f"Insertado expediente {expediente.numero} con {len(movimientos)} movimientos"
                )
