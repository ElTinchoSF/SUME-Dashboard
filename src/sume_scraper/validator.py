...
class ValidationReport:
    ...
    def add_issue(
        self, level: str, category: str, message: str, details: str | None = None
    ) -> None:
...
class ScrapingValidator:
    ...
    def _check_data_integrity(self, conn: sqlite3.Connection, report: ValidationReport) -> None:
        ...
        cursor = conn.execute("SELECT COUNT(*) FROM expedientes WHERE fecha_alta IS NULL")
        count = cursor.fetchone()[0]
        if count > 0:
    ...
    def _check_date_ranges(self, conn: sqlite3.Connection, report: ValidationReport) -> None:
        ...
        cursor = conn.execute("SELECT MIN(fecha_alta), MAX(fecha_alta) FROM expedientes")
